...
sub load_servers {
   ...
   while (my $row = $sth_servers->fetchrow_hashref) {
      ...
      print " - added server: $sid (network $nid: $network) - $host:$port " . ($pass ? "*password*" : "") . " priority $priority " . ($disabled ? " *disabled*" : "") . "\n";
...
sub send_wx {
   ...
   my $occupancy_msg = get_occupancy();
...
sub bot_start {
    ...
    print "Connecting as nick: $nick on network: $network ($nid) via server: $server\n";
    $heap->{irc}->yield(connect => { Nick => $nick, Server => $server });
    print "* EXIT bot_start\n";
}

#########
# XXX: Make these two suck less
#########
